...
PARAMS = {
    "trigger_file": Param(
        ["fact_production", "machine_status_logs", "production_records"],
        type="array",
        title="選擇執行 SQL 檔案",
...
def check_branch(**kwargs) -> list:
    ...
    _get_list = dag_run.get(
        "trigger_file", ["fact_production", "machine_status_logs", "production_records"]
    )
...
with dag:
    ...
    CHECK_BRANCH = BranchPythonOperator(
        task_id="CHECK_BRANCH", python_callable=check_branch
    )
    with TaskGroup(group_id=DAG_ID) as WF_AUTO_PARTITION:
        target_list = ["fact_production", "machine_status_logs", "production_records"]
    ...
    START >> CHECK_PARAMETERS >> CHECK_BRANCH >> WF_AUTO_PARTITION >> END
